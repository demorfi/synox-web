...
class Pornolab extends Package implements Download
{
     ...
     * @var string
     */
    const SITE_PREFIX = 'http://pornolab.net/forum';
    ...
    protected function isAvailableAccount()
    {
        ...
        if (!$html->find('a[href*="/profile.php?mode=viewprofile"]')->length) {
            ...
            $html = pqInstance($response);
            return ($html->find('a[href*="/profile.php?mode=viewprofile"]')->length ? true : false);
        }
        return (true);
    }
    ...
    public function searchByName($name, Stack $stack)
    {
        if ($this->isAvailableAccount()) {
            ...
            preg_match(
                '/(?P<result>\d+)\s+\(/',
                $html->find('#main_content .maintitle ~ .med:first')->text(),
                $matches
            );
            ...
            if ($countPages > 1) {
                for ($i = 2; $i <= $countPages; $i++) {
                    $link = $html->find('#main_content_wrap > table .menu-root ~ a:not(:last):eq(' . ($i - 2) . ')')
                        ->attr('href');
    ...
    {
        $items = [];
        foreach ($dom->find('.forumline tr.tCenter') as $item) {
            $item = pqElement($item);
            $url  = self::SITE_PREFIX . ltrim($item->find('a.tLink')->attr('href'), '.');
    ...
    protected function createItem($url, \phpQueryObject $element, \phpQueryObject $page)
    {
        ...
        // Category torrent
        $category = trim($page->find('#main_content table table .nav a:last')->text());
        $item->setCategory((!empty($category) ? $category : 'unknown'));

        // Title torrent
        $title = trim($page->find('#main_content table .maintitle a:first')->text());
        $item->setTitle((!empty($title) ? $title : 'unknown'));

        // Url download torrent
        $download = $topic->find('.attach a.dl-link')->attr('href');
        $item->setFetch((!empty($download) ? (self::SITE_PREFIX . '/' . $download) : 'unknown'));

        // Torrent size
        $item->setSize((float)$element->find('.row4.nowrap:not(:last) u')->text());
    ...
    public function fetch($url, Torrent $file)
    {
        if ($this->isAvailableAccount()) {
            ...
            $content = $this->sendPost($client, $url, $fields);
            if ($file->is($content)) {
